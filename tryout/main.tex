...
\version{0.4$\alpha$}    % wird im Entwurf auf der Titelseite vermerkt
\datum{\today}
%%
...
\section{LLM-gestützte ETL-Pipelines}
	...
	\chapter{Methode}\ref{chapter.methode}
...
Die Implementierung der LLM-gestützten Vorverarbeitung wurde in Python realisiert und als eigenständiges Paket mit klar abgegrenzten Verarbeitungsschritten umgesetzt. Das Paket wurde als \texttt{struct2prose} benannt, nach seiner Funktion, strukturierte Daten in Prosa umzuwandeln und liegt im \texttt{src}-Layout vor. Die Installation erfolgt als editierbares Paket über \texttt{pyproject.toml} und die Ausführung des Projekts erfolgt mit dem Aufruf von \texttt{python -m struct2prose}.

\subsection{Kommandozeilenschnittstelle und Pipeline-Ausführung}\label{sub.Kommandozeilenschnittstelle}
